#summary Description of the TPJBrowseDialog.Options property.
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
|| *Value* || *Meaning* ||
|| `boShowHelp` || Displays a help button in the dialog box. When the user clicks the button the application's help file is activated at the topic given by the component's _[TPJBrowseDialogHelpContext HelpContext]_ or _[TPJBrowseDialogHelpKeyword HelpKeyword]_ properties. When _[TPJBrowseDialogHelpContext HelpContext]_ is 0 or _[TPJBrowseDialogHelpKeyword HelpKeyword]_ is the empty string the button is disabled. If _Options_ does not include `boContextHelp` then pressing F1 has the same effect as clicking the _Help_ button. *Note:* This option is ignored when `boNewStyleDlg` is included in _Options_. ||
...
|| `boStatusText` || Permits status information to be displayed in the dialog box in response to selection changes. To cause this text to appear either the _[TPJBrowseDialogOnSelChange OnSelChange]_ or the _[TPJBrowseDialogOnSelChangeEx OnSelChangeEx]_ event must be handled and the required text sent back to the dialog box via the event handler's _!StatusText_ parameter. *Note:* This option is ignored when `boNewStyleDlg` is included in _Options_. ||
