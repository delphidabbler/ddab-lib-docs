...
If you don't want to use the object inspector you can double click the form to create a _!FormCreate_ event handler and enter the following code in it, which assumes the component is named _PJHotLabel1_:
...
Start a new application and drop a _[TPJHotLabel TPJHotLabel]_ component on the form. We only need to set the _[TPJHotLabelHighlightURL HighlightURL]_ property to `True`, leaving all the other properties unchanged. You can use the property inspector do this or create a _!FormCreate_ event handler and enter the following line of code, assuming the component is named _PJHotLabel1_:
