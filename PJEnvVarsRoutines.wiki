#summary Environment Variables Unit routines.
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
|| This is the documentation for the *v2.0* release of the unit. If you are using a *version 3* release please [http://wiki.delphidabbler.com/index.php/Docs/PJEnvVarsRoutines see here]. ||
...
Returns the value of the environment variable named by _!VarName_. The empty string is returned if the named variable does not exist.
...
Sets the environment variable named by _!VarName_ to _!VarValue_. If there is no usch environment variable it is created. If _!VarValue_ is the empty string then the environment variable is deleted.

Returns 0 on success or a Windows error code on error. The most likely cause of error is that the environment block is full and there is no room for the new value or to create a new variable. A description of any error can be found by passing the error code to _!SysUtils.!SysErrorMessage_.
...
Deletes the environment variable specified by _!VarName_. If the variable does not exist _!DeleteEnvVar_ does nothing.

If _!DeleteEnvVar_ completes successfuly then 0 is returned. If an error occurs then a non-zero Windows error code is returned. A description of any error can be found by passing the error code to _!SysUtils.!SysErrorMessage_.
...
It is permitted to pass nil as the parameter to _!GetAllEnvVars_. If this is done then details of the environment variables are not provided, but the size of the environment block is returned. Therefore this function can also be used to determine the size of the environment block.
...
_!ExpandEnvVars_ replaces any environment variables embedded in string _Str_ with their values and returns the modified string.
...
_!CreateEnvBlock_ creates a new environment block that can be used to pass to another process.

The new environment block is stored in the memory pointed to by _Buffer_, which is taken to be at least _!BufSize_ characters in size. The actual size of the environment block is returned by _!CreateEnvBlock_. If the buffer provided is nil or is too small then no block is created and the return value is the required buffer size in characters.

The _!NewEnv_ and _!IncludeCurrent_ parameters determine what is included in the new environment block. If the _!NewEnv_ string list is not nil then it must contain a list of environment variables in the form `NAME=VALUE`. The new environment block will contain these values. If _!IncludeCurrent_ is true then the new environment block will also include a copy of the current process' environment block.

The usual way to use _!CreateEnvBlock_ is to call it once with a nil buffer to get the required buffer size in characters, allocate the buffer and then call _!CreateEnvBlock_ again, this time passing the actual buffer and its size, for example:
...
*WARNING:* _!CreateEnvBlock_ deals with buffer sizes in characters, not bytes, which is not the same thing on Unicode Delphis. When allocating buffers in bytes (e.g. with _!GetMem_) you must multiply the required size returned from _!CreateEnvBlock_ by !SizeOf(Char). Alternatively, use a routine such as _!StrAlloc_ that automatically takes account of the character size.
