...
This protected method is overrridden by _[TPJRegWdwState TPJRegWdwState]_ to read the window's state, size and position information from the registry. Before reading the registry _!ReadWdwState_ triggers either the _[TPJRegWdwStateOnGetRegData OnGetRegData]_ or the _[TPJRegWdwStateOnGetRegDataEx OnGetRegDataEx]_*^v5.6^* event to enable the user to change the key from which the state data is to be read.
