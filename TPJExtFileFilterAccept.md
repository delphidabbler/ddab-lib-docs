#summary Description of the TPJExtFileFilter.Accept method
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
For a file to pass through the filter it must either be of a type that is not being filtered (per the _[TPJExtFileFilterStyle Style]_ property) or it must have an extension that matches one of those listed in the _[TPJExtFileFilterExtensions Extensions]_ property.
