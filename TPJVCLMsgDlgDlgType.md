...
The buttons displayed in the dialogue box can be determined by specifying one, and only one, of the following constants. Setting one of these values also updates the _[TPJVCLMsgDlgButtonGroup ButtonGroup]_ property, which in turn updates the _[TPJVCLMsgDlgButtons Buttons]_ property.
...
The kind of dialogue box (its icon, default title, etc.) is specified by ORing one of the following constants with one of those used to determine the button group. Setting one of these values updates the _[TPJVCLMsgDlgKind Kind]_ property.
...
The component of _!DlgType_ that identifies the kind of dialogue box displayed is set according to the value of the _[TPJVCLMsgDlgKind Kind]_ property as per the following table:
...
If the dialogue box will show a help button when displayed then `MB_HELP` is included in _!DlgType_. This occurs in the following circumstances, depending on the _[TPJVCLMsgDlgOptions Options]_ property:
