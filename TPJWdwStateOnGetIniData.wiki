#summary Description of the TPJWdwState.OnGetIniData event.
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
_!OnGetIniData_ allows the user to change the name of the ini file and the section within it where window information is stored.
...
The event is triggered in the following circumstances:
  ...
  * ^v5.5^ When the _[TPJWdwStateIniFilePath IniFilePath]_ method is called.

The name of the ini file and the section within it are passed as var parameters to the event handler, allowing the user to change the values, and hence the location where the window data is recorded.  The values passed to the event handler are the same as those of the _[TPJWdwStateIniFileName IniFileName] and _[TPJWdwStateSection Section]_ properties.
...
Changing the values of the _AIniFileName_ or _ASection_ parameters has exactly the same effect on the ini file and section names as setting the _[TPJWdwStateIniFileName IniFileName]_ or _[TPJWdwStateSection Section]_ properties to the same values. The full name of the ini file is also influenced by the value of the _[TPJWdwStateIniRootDir IniRootDir]_ property. See the documentation of those properties for details.
