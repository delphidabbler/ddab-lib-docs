...
Application defined data can be read from and written to the registry by handling the _[TPJRegWdwStateOnGettingRegData OnGettingRegData]_*^v5.1^* and _[TPJRegWdwStateOnPuttingRegData OnPuttingRegData]_*^v5.1^* events that are triggered when the component reads and writes its data.

The root and sub key to be used in the registry are specified using the _[TPJRegWdwStateRootKey RootKey]_ (or _[TPJRegWdwStateRootKeyEx RootKeyEx]_*^v5.6^*) and _[TPJRegWdwStateSubKey SubKey]_ properties respectively. Alternatively the _[TPJRegWdwStateOnGetRegData OnGetRegData]_ (or _[TPJRegWdwStateOnGetRegDataEx OnGetRegDataEx]_*^v5.6^*) event can be handled and used to set the root key and sub key to use.
...
|| _[TPJRegWdwStateRootKey RootKey]_ || Specifies the _HKEY_ value of the registry root key under which the window's size, position and state information is stored. ||
|| _[TPJRegWdwStateRootKeyEx RootKeyEx]_*^v5.6^* || Similar to _[TPJRegWdwStateRootKey RootKey]_ except that instead of taking an _HKEY_ value it takes a value from the _[TPJRegRootKey]_*^v5.6^* enumeration. ||
...
|| _[TPJRegWdwStateOnGetRegData OnGetRegData]_ || Event that allows user to change the registry key where window information is stored. The registry root key is specified by it _HKEY_ value. ||
|| _[TPJRegWdwStateOnGetRegDataEx OnGetRegDataEx]_*^v5.6^* || Similar to _[TPJRegWdwStateOnGetRegData OnGetRegData]_ except that the root key is specified by a value from the _[TPJRegRootKey]_*^v5.6^* enumeration.  ||
|| _[TPJRegWdwStateOnGettingRegData OnGettingRegData]_*^v5.1^* || Event that allows user to read additional registry data when the component reads window state information. ||
|| _[TPJRegWdwStateOnPuttingRegData OnPuttingRegData]_*^v5.1^* || Event that allows user to write additional registry data when the component writes window state information. ||
