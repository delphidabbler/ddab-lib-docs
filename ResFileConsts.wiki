#summary Resource File Unit Constants
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
The following flags are used to form the bitmask in a resource entry's _!MemoryFlags_ property. The first four constants in the table can or ORd together to form the bitmask. The final three constants are complements of the first three and are ANDed against the bitmask to remove their complement from it.
