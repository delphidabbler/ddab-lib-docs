#summary Description of the TPJCustomWdwState.Options property.
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
The _[TPJCustomWdwStateSave Save]_ method always records the window state and size, regardless of whether `woIgnoreState` and `woIgnoreSize` are set. The saved state and/or size is simply ignored when one or both of these option are set.
