#summary Description of the TPJResourceEntry.IsMatching methods.
<wiki:comment>
$Rev$
$Date$
</wiki:comment>
...
*Class:* _[TPJResourceEntry TPJResourceEntry]_

There are two overloaded _!IsMatching_ methods. They are both used to check if a resource matches the one represented by the _[TPJResourceEntry TPJResourceEntry]_ object on which the method is called.
...
*_Parameters:_*
  ...
  * _!ResType_: The type of the resource to be matched as either an ordinal or string type.
  * _!ResName_: The name of the resource to be matched as either an ordinal or string type. If `nil` is passed as this parameter then it is ignored when matching.
